...
import { fetchPlaceBySlug, fetchPlaces, type Destination } from "@/app/data/api";
...
export default function DestinationDetailPage({ params }: PageProps) {
  ...
  useEffect(() => {
    Promise.all([
      fetchPlaceBySlug(resolvedParams.slug),
      fetchPlaces()
    ])
      .then(async ([dest, all]) => {
        setDestination(dest);
      ...
      .catch(async () => {
        try {
          const cached = await getCachedDestination(resolvedParams.slug);
  ...
  // Stop speaking when user navigates away or tab changes
  useEffect(() => {
    return () => {
      stopSpeaking();
      stopAudioFile();
              ...
              {destination.experienceTypes && destination.experienceTypes.length > 0 && (
                <div className="flex flex-col gap-2 mt-4 border-t border-charcoal-stone/10 pt-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {destination.experienceTypes.map((ext: string, idx: number) => (
              ...
              {destination.platformFeatures && destination.platformFeatures.length > 0 && (
                <div className="flex flex-col gap-2 pt-3 border-t border-charcoal-stone/10 mt-1">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {destination.platformFeatures.map((pf: string, idx: number) => (
