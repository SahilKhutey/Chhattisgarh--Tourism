...
function mapToDestination(item: Record<string, unknown>): Destination {
  return {
    ...
    coordinates: {
      lat: item.latitude,
      lng: item.longitude,
      mapX: item.coordinates?.mapX || 45,
      mapY: item.coordinates?.mapY || 55,
    },
    heroImage: item.heroImage || item.hero_image || 'https://images.unsplash.com/photo-example',
    storyTitle: item.storyTitle || 'Regional Folklore Legends',
    story: item.description || item.history || 'Folklore lore and local narratives details...',
    timings: '08:00 AM - 06:00 PM (Everyday)',
    routes: 'Paved transit access is open from municipal capitals.',
    bestTime: item.bestSeason || 'All Seasons',
    seasonalAdvice: 'Check active rain corridors during peak monsoon.',
    safety: item.safetyInfo || 'Stick to designated paths.',
    nearby: ['Local Villages', 'Regional Waterfalls'],
    localInsights: 'Coordinate with registered community guides.',
    ecoGuidance: item.rules || 'Littering is strictly prohibited.',
