...
test.describe("Translation Workflow", () => {
  ...
  test.skip(
    ({ isMobile }) => isMobile,
    "Language switcher not in mobile nav yet",
  );
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.route("**/api/v1/translations*", async (route) => {
      await route.fulfill({
        json: { success: true, data: { "home.heading_real": "नमस्ते" } },
      });
    });
  });

  test("User can switch language and see translated content", async ({
    page,
  }) => {
  ...
  test("Language preference persists on reload", async ({ page }) => {
    ...
    // The button for "हिन्दी" should have the active class
    const hindiButton = page.getByRole("button", { name: "हिन्दी" });
    await expect(hindiButton).toHaveClass(/bg-white\/20|bg-forest-emerald/);
  });
});
